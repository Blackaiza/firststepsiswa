...
// Fetch the submitted category from the database
$submittedCategory = ''; // Initialize the variable for the category

if ($hasSubmitted) {
    $submissionQuery = $conn->prepare("SELECT fld_category FROM tbl_requests WHERE fld_user_id = ?");
    $submissionQuery->bind_param("i", $student_id);
    ...
    if ($submissionResult->num_rows > 0) {
        ...
        $submittedCategory = $submission['fld_category'];  // Save the selected category
    }
}
  ...
  <div class="form-container">
    <form action="proses_permohonan.php" method="POST" enctype="multipart/form-data">
      ...
      <div class="form-group">
        <label>Nama:</label>
        <input type="text" name="fld_name" class="form-control" required maxlength="300" pattern="^(\b\w+\b[\s\r\n]*){1,50}$" placeholder="Contoh: Nor Amalia Binti Nor Bani" value="<?= htmlspecialchars($name) ?>" <?= $hasSubmitted ? 'disabled' : '' ?>>
      </div>

      <div class="form-group">
        <label>No Matrik:</label>
        <input type="text" name="fld_matric_no" class="form-control" required pattern="^A\d{6}$" placeholder="Contoh: A123456" value="<?= htmlspecialchars($matric_no) ?>" <?= $hasSubmitted ? 'disabled' : '' ?>>
      </div>

      <div class="form-group">
        <label>No Telefon:</label>
        <input type="text" name="fld_phone" class="form-control" required pattern="^01\d{8,9}$" placeholder="Contoh: 01123345672" maxlength="11" inputmode="numeric" value="<?= htmlspecialchars($phone) ?>" <?= $hasSubmitted ? 'disabled' : '' ?>>
      </div>

      <div class="form-group">
        <label>Email:</label>
        <input type="email" name="fld_email" class="form-control" required pattern="^[a-zA-Z0-9._%+-]+@siswa\.ukm\.edu\.my$" placeholder="Contoh: nomatrik@siswa.ukm.edu.my" value="<?= htmlspecialchars($email) ?>" <?= $hasSubmitted ? 'disabled' : '' ?>>
      </div>

      <div class="form-group">
        <label>Jenis Bantuan:</label>
        <select name="fld_category" class="form-control" required <?= $hasSubmitted ? 'disabled' : '' ?>>
          <option value="" disabled selected>-- Pilih Bantuan --</option>
          <option value="Peralatan Digital" <?php if($submittedCategory == "Peralatan Digital") echo "selected"; ?>>Peralatan Digital</option>
          <option value="Bahan Pembelajaran" <?php if($submittedCategory == "Bahan Pembelajaran") echo "selected"; ?>>Bahan Pembelajaran</option>
          <option value="Keperluan Diri" <?php if($category == "Keperluan Diri") echo "selected"; ?>>Keperluan Diri</option>
          <option value="Peralatan Digital + Bahan Pembelajaran" <?php if($submittedCategory == "Peralatan Digital + Bahan Pembelajaran") echo "selected"; ?>>Peralatan Digital + Bahan Pembelajaran</option>
          <option value="Bahan Pembelajaran + Keperluan Diri" <?php if($submittedCategory == "Bahan Pembelajaran + Keperluan Diri") echo "selected"; ?>>Bahan Pembelajaran + Keperluan Diri</option>
          <option value="Peralatan Digital + Keperluan Diri" <?php if($submittedCategory == "Peralatan Digital + Keperluan Diri") echo "selected"; ?>>Peralatan Digital + Keperluan Diri</option>
          <option value="Peralatan Digital + Bahan Pembelajaran + Keperluan Diri" <?php if($submittedCategory == "Peralatan Digital + Bahan Pembelajaran + Keperluan Diri") echo "selected"; ?>>Peralatan Digital + Bahan Pembelajaran + Keperluan Diri</option>
        </select>
      </div>
      ...
      <div class="form-group">
        ...
        <?php endif; ?>
        <input type="file" name="fld_income_slip" class="form-control-file" accept=".pdf" <?= $hasSubmitted ? 'disabled' : '' ?> <?= empty($income_slip_path) ? 'required' : '' ?>>
      </div>
      ...
      <div class="form-group">
        ...
        <?php endif; ?>
        <input type="file" name="fld_supporting_doc" class="form-control-file" accept=".pdf" <?= $hasSubmitted ? 'disabled' : '' ?> <?= empty($support_doc_path) ? 'required' : '' ?>>
      </div>

      <div class="form-group d-flex justify-content-between align-items-center">
        <div>
          <button type="submit" name="save_draft" class="btn btn-primary" <?= $hasSubmitted ? 'disabled' : '' ?>>Simpan Draf</button>
